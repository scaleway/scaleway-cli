...
| name |  |  |
| priority |  |  |
| ttl | Required, Default: `3600` |  |
| type | Required, One of: `A`, `AAAA`, `CNAME`, `TXT`, `SRV`, `TLSA`, `MX`, `NS`, `PTR`, `CAA`, `ALIAS`, `LOC`, `SSHFP`, `HINFO`, `RP`, `URI`, `DS`, `NAPTR` |  |
| comment |  |  |
...
| format | Default: `bind`, One of: `unknown_raw_format`, `bind` | DNS zone format |
...
| ~~content~~ | Deprecated |  |
| project-id |  | Project ID to use. If none is passed the default project ID will be used |
| ~~format~~ | Deprecated, One of: `unknown_raw_format`, `bind` |  |
| bind-source.content |  |  |
