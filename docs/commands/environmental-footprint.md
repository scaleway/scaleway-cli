...
| service-categories.{index} | One of: `unknown_service_category`, `baremetal`, `compute`, `storage`, `network`, `containers`, `databases`, `ai`, `serverless` | List of service categories to filter by. Defaults to all service categories. |
| product-categories.{index} | One of: `unknown_product_category`, `apple_silicon`, `block_storage`, `dedibox`, `elastic_metal`, `instances`, `object_storage`, `load_balancer`, `kubernetes`, `managed_relational_databases`, `managed_mongodb`, `managed_redis`, `managed_inference`, `generative_apis`, `serverless_functions`, `serverless_containers`, `serverless_jobs` | List of product categories to filter by. Defaults to all product categories. |
