package account

import (
	"io/ioutil"
	"os"
	"path"
	"testing"

	"github.com/scaleway/scaleway-cli/internal/core"
)

func Test_initCommand(t *testing.T) {
	t.Run("simple", core.Test(&core.TestConfig{
		Commands: GetCommands(),
		BeforeFunc: func(ctx *core.BeforeFuncCtx) error {
			tmpDir := os.TempDir()
			pathToPublicKey := path.Join(tmpDir, ".ssh", "id_rsa.pub")
			pathShort := path.Join(tmpDir, "id_rsa.pub")
			_, err := os.Stat(pathToPublicKey)
			if err != nil {
				content := "ssh-rsa  email"
				err := os.MkdirAll(path.Join(tmpDir, ".ssh"), 0644)
				if err != nil {
					return err
				}

				err = os.MkdirAll(path.Join(tmpDir, "bob"), 0644)
				if err != nil {
					return err
				}

				// Test fails here
				err = os.MkdirAll(path.Join(tmpDir, "bob", "coucou"), 0644)
				if err != nil {
					// local error:
					// mkdir /var/folders/wh/z_3kj3ks32dfj9x58_b9c4s40000gn/T/bob/coucou: permission denied
					return err
				}

				err = os.MkdirAll(path.Join(tmpDir, ".ssh", "id_rsa.pub"), 0644)
				if err != nil {
					return err
				}

				err = ioutil.WriteFile(pathShort, []byte(content), 0644)
				if err != nil {
					return err
				}
				// gives following error
				//  open /var/folders/wh/z_3kj3ks32dfj9x58_b9c4s40000gn/T/.ssh/id_rsa.pub: permission denied
				err = ioutil.WriteFile(pathToPublicKey, []byte(content), 0644)
				return err
			}
			return err
		},
		Cmd: `scw account ssh-key init with-ssh-key=true`,
		Check: core.TestCheckCombine(
			core.TestCheckGolden(),
			core.TestCheckExitCode(0),
		),
		OverrideEnv: map[string]string{
			"HOME": os.TempDir(),
		},
	}))
}
