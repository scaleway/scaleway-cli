...
func listResources() *core.Command {
	return &core.Command{
		...
		ArgSpecs: core.ArgSpecs{
			...
			{
				Name:       "products",
				Short:      `Filter by products (comma-separated: instances,instance-ips,instance-volumes,instance-snapshots,ipam,block-volumes,block-snapshots,buckets,rdb,redis,lb,k8s,containers,functions,flexibleip,secrets,vpc,file,webhosting,registry,searchdb). If empty, all products are queried`,
				Required:   false,
				Deprecated: false,
				Positional: false,
			},
...
// SortResults sorts results by locality, then namespace, then resource, then ID for consistent output
