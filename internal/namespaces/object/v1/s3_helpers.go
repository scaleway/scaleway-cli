...
func getS3Endpoints(
	...
	if customEndpoint != "" {
		...
		if s3EndpointOk && profileS3Endpoint != "" {
			...
			// Possible edge case: a user who explicitly sets s3_endpoint:
			// https://s3.fr-par.scw.cloud in profile and runs --region nl-ams
			// will silently get nl-ams endpoint instead of their configured one
