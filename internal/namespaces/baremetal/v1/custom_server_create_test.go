...
func Test_CreateServer(t *testing.T) {
	...
	t.Run("Simple", func(t *testing.T) {
		t.Run("Default", core.Test(&core.TestConfig{
			...
			Cmd:      "scw baremetal server create zone=fr-par-1 type=EM-L101X-SATA -w",
		...
		t.Run("With name", core.Test(&core.TestConfig{
			Commands: GetCommands(),
			Cmd:      "scw baremetal server create name=test-create-server-with-name zone=fr-par-1 type=EM-L101X-SATA -w",
			Check: core.TestCheckCombine(
		...
		t.Run("Tags", core.Test(&core.TestConfig{
			Commands: GetCommands(),
			Cmd:      "scw baremetal server create tags.0=prod tags.1=blue zone=fr-par-1 type=EM-L101X-SATA -w",
			Check: core.TestCheckCombine(
				func(t *testing.T, ctx *core.CheckFuncCtx) {
