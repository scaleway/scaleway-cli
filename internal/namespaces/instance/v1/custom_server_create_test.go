...
func Test_CreateServer(t *testing.T) {
	...
	t.Run("Simple", func(t *testing.T) {
		t.Run("Default", core.Test(&core.TestConfig{
			...
			Cmd:      "scw instance server create image=ubuntu_focal stopped=true",
		...
		t.Run("GP1-XS", core.Test(&core.TestConfig{
			Commands: GetCommands(),
			Cmd:      "scw instance server create type=GP1-XS image=ubuntu_focal stopped=true",
			Check: core.TestCheckCombine(
				func(t *testing.T, ctx *core.CheckFuncCtx) {
		...
		t.Run("With name", core.Test(&core.TestConfig{
			Commands: GetCommands(),
			Cmd:      "scw instance server create image=ubuntu_focal name=yo stopped=true",
			Check: core.TestCheckCombine(
				func(t *testing.T, ctx *core.CheckFuncCtx) {
		...
		t.Run("With start", core.Test(&core.TestConfig{
			Commands: GetCommands(),
			Cmd:      "scw instance server create image=ubuntu_focal -w",
			Check: core.TestCheckCombine(
				func(t *testing.T, ctx *core.CheckFuncCtx) {
		...
		t.Run("With bootscript", core.Test(&core.TestConfig{
			Commands: GetCommands(),
			Cmd:      "scw instance server create image=ubuntu_focal bootscript-id=eb760e3c-30d8-49a3-b3ad-ad10c3aa440b stopped=true",
			Check: core.TestCheckCombine(
				func(t *testing.T, ctx *core.CheckFuncCtx) {
		...
		t.Run("Tags", core.Test(&core.TestConfig{
			Commands: GetCommands(),
			Cmd:      "scw instance server create image=ubuntu_focal tags.0=prod tags.1=blue stopped=true",
			Check: core.TestCheckCombine(
				func(t *testing.T, ctx *core.CheckFuncCtx) {
	...
	t.Run("Volumes", func(t *testing.T) {
		...
		t.Run("valid single local volume", core.Test(&core.TestConfig{
			Commands: GetCommands(),
			Cmd:      "scw instance server create image=ubuntu_focal root-volume=local:20GB stopped=true",
			Check: core.TestCheckCombine(
				func(t *testing.T, ctx *core.CheckFuncCtx) {
		...
		t.Run("valid double local volumes", core.Test(&core.TestConfig{
			Commands: GetCommands(),
			Cmd:      "scw instance server create image=ubuntu_focal root-volume=local:10GB additional-volumes.0=l:10G stopped=true",
			Check: core.TestCheckCombine(
				func(t *testing.T, ctx *core.CheckFuncCtx) {
		...
		t.Run("valid additional block volumes", core.Test(&core.TestConfig{
			Commands: GetCommands(),
			Cmd:      "scw instance server create image=ubuntu_focal additional-volumes.0=b:1G additional-volumes.1=b:5G additional-volumes.2=b:10G stopped=true",
			Check: core.TestCheckCombine(
				func(t *testing.T, ctx *core.CheckFuncCtx) {
	...
	t.Run("IPs", func(t *testing.T) {
		...
		t.Run("explicit new IP", core.Test(&core.TestConfig{
			Commands: GetCommands(),
			Cmd:      "scw instance server create image=ubuntu_focal ip=new stopped=true",
			Check: core.TestCheckCombine(
				func(t *testing.T, ctx *core.CheckFuncCtx) {
		...
		t.Run("run with dynamic IP", core.Test(&core.TestConfig{
			Commands: GetCommands(),
			Cmd:      "scw instance server create image=ubuntu_focal ip=dynamic -w", // dynamic IP is created at runtime
			Check: core.TestCheckCombine(
				func(t *testing.T, ctx *core.CheckFuncCtx) {
		...
		t.Run("existing IP", core.Test(&core.TestConfig{
			...
			Cmd:        "scw instance server create image=ubuntu_focal ip={{ .IP.Address }} stopped=true",
			Check: core.TestCheckCombine(
				func(t *testing.T, ctx *core.CheckFuncCtx) {
					assert.NotEmpty(t, ctx.Result.(*instance.Server).PublicIP.Address)
					assert.Equal(t, false, ctx.Result.(*instance.Server).PublicIP.Dynamic)
		...
		t.Run("existing IP ID", core.Test(&core.TestConfig{
			Commands:   GetCommands(),
			BeforeFunc: createIP("IP"),
			Cmd:        "scw instance server create image=ubuntu_focal ip={{ .IP.ID }} stopped=true",
			Check: core.TestCheckCombine(
				func(t *testing.T, ctx *core.CheckFuncCtx) {
					assert.NotEmpty(t, ctx.Result.(*instance.Server).PublicIP.Address)
					assert.Equal(t, false, ctx.Result.(*instance.Server).PublicIP.Dynamic)
		...
		t.Run("with ipv6", core.Test(&core.TestConfig{
			Commands: GetCommands(),
			Cmd:      "scw instance server create image=ubuntu_focal ipv6=true -w", // IPv6 is created at runtime
			Check: core.TestCheckCombine(
				func(t *testing.T, ctx *core.CheckFuncCtx) {
...
func Test_CreateServerErrors(t *testing.T) {
	...
	t.Run("Error: invalid instance type", core.Test(&core.TestConfig{
		Commands: GetCommands(),
		Cmd:      "scw instance server create type=MACBOOK1-S image=ubuntu_focal",
		Check: core.TestCheckCombine(
			core.TestCheckGolden(),
			core.TestCheckExitCode(1),
		),
	...
	t.Run("Error: invalid total local volumes size: too low 1", core.Test(&core.TestConfig{
		Commands: GetCommands(),
		Cmd:      "scw instance server create image=ubuntu_focal root-volume=l:10GB",
		Check: core.TestCheckCombine(
			core.TestCheckGolden(),
			core.TestCheckExitCode(1),
		),
	...
	t.Run("Error: invalid total local volumes size: too low 2", core.Test(&core.TestConfig{
		Commands: GetCommands(),
		Cmd:      "scw instance server create image=ubuntu_focal root-volume=l:10GB additional-volumes.0=block:10GB",
		Check: core.TestCheckCombine(
			core.TestCheckGolden(),
			core.TestCheckExitCode(1),
		),
	...
	t.Run("Error: invalid total local volumes size: too high 1", core.Test(&core.TestConfig{
		Commands: GetCommands(),
		Cmd:      "scw instance server create image=ubuntu_focal root-volume=local:10GB additional-volumes.0=local:20GB",
		Check: core.TestCheckCombine(
			core.TestCheckGolden(),
			core.TestCheckExitCode(1),
		),
	...
	t.Run("Error: invalid total local volumes size: too high 2", core.Test(&core.TestConfig{
		Commands: GetCommands(),
		Cmd:      "scw instance server create image=ubuntu_focal additional-volumes.0=local:10GB",
		Check: core.TestCheckCombine(
			core.TestCheckGolden(),
			core.TestCheckExitCode(1),
		),
	...
	t.Run("Error: invalid total local volumes size: too high 3", core.Test(&core.TestConfig{
		...
		Cmd:        "scw instance server create image=ubuntu_focal root-volume={{ .Volume.ID }} additional-volumes.0=local:10GB",
		Check: core.TestCheckCombine(
			core.TestCheckGolden(),
			core.TestCheckExitCode(1),
		),
	...
	t.Run("Error: invalid root volume size", core.Test(&core.TestConfig{
		Commands: GetCommands(),
		Cmd:      "scw instance server create image=ubuntu_focal root-volume=local:2GB additional-volumes.0=local:18GB",
		Check: core.TestCheckCombine(
			core.TestCheckGolden(),
			core.TestCheckExitCode(1),
		),
	...
	t.Run("Error: invalid root volume type", core.Test(&core.TestConfig{
		Commands: GetCommands(),
		Cmd:      "scw instance server create image=ubuntu_focal root-volume=block:20GB",
		Check: core.TestCheckCombine(
			core.TestCheckGolden(),
			core.TestCheckExitCode(1),
		),
	...
	t.Run("Error: disallow existing root volume ID", core.Test(&core.TestConfig{
		Commands:   GetCommands(),
		BeforeFunc: createVolume("Volume", 20, instance.VolumeTypeLSSD),
		Cmd:        "scw instance server create image=ubuntu_focal root-volume={{ .Volume.ID }}",
		Check: core.TestCheckCombine(
			core.TestCheckGolden(),
			core.TestCheckExitCode(1),
		),
	...
	t.Run("Error: invalid root volume ID", core.Test(&core.TestConfig{
		Commands: GetCommands(),
		Cmd:      "scw instance server create image=ubuntu_focal root-volume=29da9ad9-e759-4a56-82c8-f0607f93055c",
		Check: core.TestCheckCombine(
			core.TestCheckGolden(),
			core.TestCheckExitCode(1),
		),
	...
	t.Run("Error: already attached additional volume ID", core.Test(&core.TestConfig{
		Commands:   GetCommands(),
		BeforeFunc: core.ExecStoreBeforeCmd("Server", "scw instance server create name=cli-test image=ubuntu_focal root-volume=l:10G additional-volumes.0=l:10G stopped=true"),
		Cmd:        `scw instance server create image=ubuntu_focal root-volume=l:10G additional-volumes.0={{ (index .Server.Volumes "1").ID }} stopped=true`,
		Check: core.TestCheckCombine(
			core.TestCheckGolden(),
			core.TestCheckExitCode(1),
		),
	...
	t.Run("Error: invalid root volume format", core.Test(&core.TestConfig{
		Commands: GetCommands(),
		Cmd:      "scw instance server create image=ubuntu_focal root-volume=20GB",
		Check: core.TestCheckCombine(
			core.TestCheckGolden(),
			core.TestCheckExitCode(1),
		),
	...
	t.Run("Error: not found ip ID", core.Test(&core.TestConfig{
		Commands: GetCommands(),
		Cmd:      "scw instance server create image=ubuntu_focal ip=23165951-13fd-4a3b-84ed-22c2e96658f2",
		Check: core.TestCheckCombine(
			core.TestCheckGolden(),
			core.TestCheckExitCode(1),
		),
	...
	t.Run("Error: forbidden IP", core.Test(&core.TestConfig{
		Commands: GetCommands(),
		Cmd:      "scw instance server create image=ubuntu_focal ip=51.15.242.82",
		Check: core.TestCheckCombine(
			core.TestCheckGolden(),
			core.TestCheckExitCode(1),
		),
	...
	t.Run("Error: invalid ip", core.Test(&core.TestConfig{
		Commands: GetCommands(),
		Cmd:      "scw instance server create image=ubuntu_focal ip=yo",
		Check: core.TestCheckCombine(
			core.TestCheckGolden(),
			core.TestCheckExitCode(1),
		),
