...
func logGetBuilder(c *core.Command) *core.Command {
	c.Short = `Fetch logs from a database instance`
	c.Long = `Prepare, wait for, and download logs from a Database Instance. This command automatically prepares the logs, waits for them to be ready, and downloads them to a file.`
	c.ArgsType = reflect.TypeOf(logGetArgs{})
	...
	c.Run = func(ctx context.Context, argsI any) (i any, err error) {
		args := argsI.(*logGetArgs)
		api := rdb.NewAPI(core.ExtractClient(ctx))
