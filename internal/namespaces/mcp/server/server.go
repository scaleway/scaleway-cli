...
// RegisterCommandAsTool registers a CLI command as an MCP tool.
// This is the main entry point used by mcp.go
func RegisterCommandAsTool(server *mcp.Server, cmd *core.Command) error {
	// Skip hidden commands
	if cmd.Hidden {
		return nil
	}
	...
	// Register with MCP SDK
	mcp.AddTool(server, mcpTool, wrapper)
