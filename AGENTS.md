...
mise run lint:cli -- --fix              # auto-fix lint issues (runs golangci-lint --fix)

# Run tests
mise run test:cli
mise run test:cli -- -run <regex>       # run specific tests
mise run test:cli -- -goldens           # update golden files
mise run test:cli -- -cassettes         # record new cassettes (requires valid API credentials)
mise run test:cli -- -debug             # enable debug mode
