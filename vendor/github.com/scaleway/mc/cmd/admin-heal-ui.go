...
func round(d time.Duration, m time.Duration) time.Duration {
	if m <= 0 {
		return d
	}
	r := d % m
...
func (ui *uiData) getProgress() (oCount, objSize, duration string) {
	...
	duration = round(ui.HealDuration, time.Second).String()
...
func (ui *uiData) printStatsJSON(s *madmin.HealTaskStatus) {
	...
	summary.ElapsedTime = int64(round(ui.HealDuration, time.Second).Seconds())
