...
program
  ...
  .option('-a, --author <author>', 'author (e.g., "Georges Abitbol <georges@most-class.world>")')
...
program
  ...
  .option('-f, --filter <filters>', 'provide filter values. valid filters: (i.e., status=pending)', utils.collect, [])
  ...
  .action(function(server, command, commandArgs, options) {
    ...
    client.get('/servers/' + server)
      .then(function(res) {
        ...
        var args = [].concat.apply(['-l', 'root', ip, '-t', '--', command], commandArgs);
...
program
  ...
  .option('-f, --filter <filters>', 'provide filter values. (i.e., \'public=true\')', utils.collect, [])
  .option('--no-trunc', 'don\'t truncate output')
  .option('-q, --quiet', 'only display numeric IDs')
  .action(function(options) {
    var client = utils.newApi(options);
