...
program
  ...
  .action(function(server, name, options) {
    ...
    if (options.author || options.pause) {
      utils.panic("scw: Not implemented option");
    }
    ...
    Q.all(_.map(options.volume, function(volume) {
      ...
      function(results) {
        ...
        utils.searchEntity({input: options.bootscript, _type: 'bootscripts'}, function(err, bootscriptEntity) {
          options.bootscript = bootscriptEntity && bootscriptEntity.id;

          // Resolve image
          utils.searchEntity({input: image, _type: 'images'}, function(err, imageEntity) {
            if (err) {
    ...
    utils.searchEntity({input: server, _type: 'servers'}, function(err, entity) {
      ...
      client.get('/servers/' + entity._id)
        .then(function(res) {
          ...
          if (options.insecure || client.config.exec_insecure) {
            args = [].concat.apply([
