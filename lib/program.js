...
program
  ...
  .action(function(image, options) {
    ...
    utils.searchEntity({
      ...
      utils.searchEntity({
        input: options.bootscript,
        filters: {
          _type: 'bootscripts'
        }
      }, function(err, bootscriptEntity) {
        if (options.bootscript && err) {
          utils.panic(err);
        }
        ...
        if (options.bootscript) {
          data.bootscript = bootscriptEntity._id;
        }
...
program
  ...
  .action(function(server, options) {
    ...
    utils.searchEntity({
      ...
      var latest_state = 'unknown';
      async.whilst(
        function(a) {
          return latest_state !== 'stopped';
        },
        function(cb) {
          client.get('/servers/' + entity._id)
            .then(function(res) {
              latest_state = res.body.server.state;
